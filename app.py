...
def get_question_by_id(language, id_):
    ...
    if int(id_) > 10 and language == 'ruby':
        ...
        button = Markup(f'<form method="GET" action="/questionlevel2/python/11"><button class="submitbutton" type="submit">Go on level 2! ✌️</button></form>')
        flash("Well done! You completed the Python Level 1! 🎉")
        flash(button)
        return redirect(url_for('congratulationlevel1'))
    elif int(id_) > 30 and language == 'javascript':
        button = Markup(f'<form method="GET" action="/questionlevel2/javascript/21"><button class="submitbutton" type="submit">Go on level 2! ✌️</button></form>')
...
# ------------ PYTHON QUESTIONS PART 2 ------------
    db.session.add(QuestionLevel2(language=u'Python', question_display_id=11, question=u' 1️⃣ What is Python?', answer=u'Programming language'))
...
# ------------ JAVASCRIPT QUESTIONS PART 2 ------------
    db.session.add(QuestionLevel2(language=u'JavaScript', question_display_id=21, question=u' 1️⃣ What is JavaScript?', answer=u'programming language'))
    db.session.add(QuestionLevel2(language=u'JavaScript', question_display_id=22, question=u'2️⃣ Writing console.log(10 * 2), would the output be a String or an Integer?', answer=u'integer'))
    db.session.add(QuestionLevel2(language=u'JavaScript', question_display_id=23, question=u'3️⃣ Given the variable let javascript = "Programming Language", how do you log the value of the variable javascript?', answer=u'console.log(javascript)'))
    db.session.add(QuestionLevel2(language=u'JavaScript', question_display_id=24, question=u'4️⃣ What is the method to reverse the order of a string?', answer=u'.reverse()'))
    db.session.add(QuestionLevel2(language=u'JavaScript', question_display_id=25, question=u'5️⃣ How would you log on the console the integer 42 as a string?', answer=u'console.log("42")'))
    db.session.add(QuestionLevel2(language=u'JavaScript', question_display_id=26, question=u'6️⃣ Given an array let list = [15, "Bike", True], what would be the output of console.log(list[0])?', answer=u'15'))
    db.session.add(QuestionLevel2(language=u'JavaScript', question_display_id=27, question=u'7️⃣ Given an array let list = ["Cat", 2, "dog"], if you run the command list.push(true), what would be the output of console.log(list[3]) ?', answer=u'true'))
    db.session.add(QuestionLevel2(language=u'JavaScript', question_display_id=28, question=u'8️⃣ Given the following hash h = {"marvel":  "Iron Man", "Dc Comics": "Batman"}, what would be the output of console.log(h.marvel)', answer=u'Iron Man'))
    db.session.add(QuestionLevel2(language=u'JavaScript', question_display_id=29, question=u'9️⃣ Given the string let string = "Hello everyone!", how would you output its length?', answer=u'console.log(string.length)'))
    db.session.add(QuestionLevel2(language=u'JavaScript', question_display_id=30, question=u'🔟 Given the string let string = "amazing", how would you log on the console the value in upper case? ', answer=u'console.log(string.toUpperCase())'))
